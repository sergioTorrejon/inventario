import {
  saveFile,
  validationFileSize,
} from 'src/common/file-manager/file-validation';
import {
  CC,
  RA,
} from 'src/common/helpers/const';
import {
  Accion,
  first,
  formatYear,
  Status,
} from 'src/common/helpers/params';
import {
  FAIL,
  FAIL_FILE_SIZE,
  FAIL_NAME,
...
export class CartasResolucionesService {
  ...
  //#region REPOSITORIO
    ...
    async queryFilter(dto: CartasResolucionesSearchDto){
      ...
      //dto.modulo == ''? filter= filter +  ` AND usuario_creacion = '${dto.user}' OR usuario = '${dto.user}'`:'';
